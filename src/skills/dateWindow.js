/**
 * Date Window Skill
 * Computes PST newsletter window for a given Month.
 * 
 * Output:
...
/**
 * Get the first occurrence of a specific day in a month
 * @param {number} year 
 * @param {number} month - 0-indexed month
 * @param {number} targetDay - 0 = Sunday, 1 = Monday, ..., 6 = Saturday
 * @returns {Date}
 */
function getFirstDayOfMonth(year, month, targetDay) {
...
/**
 * Format date to ISO-8601 with Pacific offset
 * @param {Date} date 
 * @param {string} time - "00:00:00" or "23:59:59"
 * @returns {string}
 */
function formatPST(date, time) {
...
/**
 * Compute the newsletter date window
 * @param {string} monthInput - Month string like "February 2026"
 * @returns {{ month: string, startPST: string, endPST: string }}
 */
export function computeDateWindow(monthInput) {
  // Parse month input
  const months = [
...
export const dateWindowSkill = {
  ...
  execute: async ({ month }) => {
    return computeDateWindow(month);
  }
};
