...
3. Call createCommunityNews with the processed items and pass hasMore: true if more batches remain
...
5. Process the next batch and call createCommunityNews with only that batch's items and hasMore value
...
Never pass prior newsletter HTML to createCommunityNews. The extension securely replaces stale Community output on the first batch of a generation and accumulates later batches in trusted extension state. If the participant offers "Continue pending Community News" after a safety stop, use that action to resume the same generation.
...
Each call returns a structured Community batch containing only the new escaped items. Do not copy prior output into later calls; the extension output writer performs the trusted first-batch replacement and continuation accumulation.
...
5. **USER OVERRIDES DEFAULTS** - The date window is the default inclusion policy, not an absolute restriction. If the user explicitly asks to include a specific supplied post outside the window, call createProductGroupNews with includeOutsideDateWindow: true and dateWindowOverrideReason summarizing that instruction. Never infer an override yourself.
...
export type SkillPromptKey = keyof typeof SKILL_PROMPTS;

export function detectRequiredSkills(message: string): SkillPromptKey[] {
  const lowerMessage = message.toLowerCase();
  const skills: SkillPromptKey[] = [];
  ...
  // Full generation must be explicit. A month-specific newsletter or template
  // request initializes the artifact without forcing every section workflow.
  if (
    lowerMessage.includes('full newsletter') ||
    lowerMessage.includes('entire newsletter') ||
    lowerMessage.includes('all newsletter sections') ||
    lowerMessage.includes('generate all sections')
  ) {
...
function isSkillPromptKey(key: string): key is SkillPromptKey {
  return Object.prototype.hasOwnProperty.call(SKILL_PROMPTS, key);
}
